...
Para ejecutar en un dispositivo Android como Redmi Note 13:
1. Instalar **Pydroid 3** o **Termux** para tener entorno Python.
2. Instalar dependencias necesarias:
   pip install opencv-python mediapipe pandas streamlit
3. Conceder permisos de cámara a la aplicación que ejecute Python.
4. Guardar este archivo como `detector_pose_web.py`.
5. Ejecutar en la terminal:
   streamlit run detector_pose_web.py --server.headless true --server.enableCORS false
6. Abrir el navegador en el teléfono y visitar la URL local que indique Streamlit (normalmente http://localhost:8501 o la IP de la red local si accedes desde otro dispositivo).

Nota: El rendimiento puede variar según la potencia del móvil.
"""
